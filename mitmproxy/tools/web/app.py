    ...
    if isinstance(flow, http.HTTPFlow):
        if flow.request:
            ...
            if flow.request.raw_content:
                ...
                try:
                    content = etree.fromstring(flow.request.content)
                    libelle = content.xpath("local-name(//soapenv:Body/*[1])", namespaces={'soapenv' : 'http://schemas.xmlsoap.org/soap/envelope/'})
                except Exception as e:
                    libelle = flow.request.path
                    pass
            else:
                content_length = None
                content_hash = None
